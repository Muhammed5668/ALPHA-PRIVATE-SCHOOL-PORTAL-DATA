...
function renderTeacherChatList() {
    ...
    if (!teacherChatSelectedClass) {
        ...
        grades.forEach(g => {
            ...
            div.innerHTML = `<div class="c-name" style="font-size: 1.1rem; font-weight: bold; text-align: center;">${g}</div>`;
            c.appendChild(div);
        });
        ...
        backBtn.style = 'margin-bottom: 10px; width: 100%;';
        backBtn.innerText = '⬅️ Back to Classes';
        ...
        sections.forEach(s => {
            ...
            div.innerHTML = `<div class="c-name" style="font-size: 1.1rem; font-weight: bold; text-align: center;">Section ${s}</div>`;
            c.appendChild(div);
        });
    } else {
        // Show Students
        ...
        backBtn.className = 'glass-btn secondary-btn';
        backBtn.style = 'margin-bottom: 10px; width: 100%;';
        backBtn.innerText = '⬅️ Back to Sections';
        ...
        if (visibleStudents.length === 0) {
            c.insertAdjacentHTML('beforeend', '<div class="empty-state" style="margin-top:20px;padding:12px;">No students found in this section.</div>'); 
            return;
        }
        ...
        visibleStudents.forEach(s => {
            ...
            div.innerHTML = `<div class="c-name">${s.name}</div><div class="c-sub">${s.grade} · ${s.section}</div>`;
            c.appendChild(div);
        });
    }
}
    ...
    pane.innerHTML = `
        <div class="chat-active-view">
            <div class="chat-active-header">
                <button class="chat-back-btn" onclick="closeTeacherMobileChat()" aria-label="Back to chats"><svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2.2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"></path></svg></button>
                <div class="chat-avatar chat-header-avatar">${student.name.split(' ').map(w=>w[0]).slice(0,2).join('').toUpperCase()}</div>
                <h4>${student.name}</h4>
            </div>
            <div id="teacherChatMessagesBox" class="chat-messages"></div>
            <div class="chat-input-bar">
                <button class="chat-icon-btn" title="Upload Image">
                    🖼️<input type="file" accept="image/*" onchange="handleTeacherChatImageUpload(event)">
                </button>
                <input id="teacherChatConsoleInput" class="chat-text-input" type="text"
                    placeholder="Send response to student..." onkeydown="if(event.key==='Enter') sendTeacherTextMessage()">
                <button class="chat-send-btn" onclick="sendTeacherTextMessage()">Send Response</button>
            </div>
        </div>`;
...
function renderTeacherMessagesThread() {
    ...
    if (!msgs.length) { box.innerHTML = '<div class="empty-state" style="padding-top:30px;">No messages yet. Start the conversation.</div>'; return; }
...
function renderStudentAnnouncementsFeed() {
    const feed = el('studentAnnouncementsFeed'); feed.innerHTML = '';
    ...
    [...stateDatabase.announcements].reverse().forEach(a => {
        ...
        card.innerHTML = `<div class="ann-meta"><span>🏫 School Admin Office</span><span>${a.date}</span></div>
...
function renderStudentChatTeachersDirectory() {
    ...
    if (s) {
        ...
        groupDiv.innerHTML = `<div class="chat-avatar group">${shortGrade}${s.section || ''}</div><div class="c-body"><div class="c-name">${shortGrade} • ${s.section || ''}</div><div class="c-sub">Class Group Chat</div></div>`;
    ...
    visibleTeachers.forEach(t => {
        ...
        div.innerHTML = `<div class="chat-avatar">${initials}</div><div class="c-body"><div class="c-name">${t.name}</div><div class="c-sub">${t.subject} Department</div></div>`;
        c.appendChild(div);
    });
}
...
function selectTeacherForStudentConversation(teacherId) {
    ...
    pane.innerHTML = `
        <div class="chat-active-view">
            <div class="chat-active-header">
                <button class="chat-back-btn" onclick="closeStudentMobileChat()" aria-label="Back to chats"><svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2.2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M15 18l-6-6 6-6"></path></svg></button>
                <div class="chat-avatar chat-header-avatar">${teacher.name.split(' ').map(w=>w[0]).slice(0,2).join('').toUpperCase()}</div>
                <h4>${teacher.name}</h4>
            </div>
            <div id="chatMessagesBox" class="chat-messages"></div>
            <div class="chat-input-bar">
                <button class="chat-icon-btn" title="Upload Image Asset">
                    🖼️<input type="file" accept="image/*" onchange="handleStudentChatImageUpload(event)">
                </button>
                <button id="chatMicBtn" class="chat-icon-btn" onclick="toggleStudentVoiceRecordingSim()" title="Record Audio">🎤</button>
                <input id="chatConsoleInput" class="chat-text-input" type="text"
                    placeholder="Type message..." onkeydown="if(event.key==='Enter') sendStudentTextMessage()">
                <button class="chat-send-btn" onclick="sendStudentTextMessage()">Send</button>
            </div>
        </div>`;
    ...
    pane.innerHTML = `
        <div class="chat-active-view">
            <div class="chat-active-header">
                ...
                <button class="chat-group-title-btn" onclick="toggleClassChatMembersPanel()">
                    <h4>${shortGrade} • ${s.section}</h4>
                    <span>Class Group Chat — tap for members</span>
                </button>
            </div>
            ...
            <div class="chat-input-bar">
                <input id="classChatInput" class="chat-text-input" type="text" placeholder="Message the class..." onkeydown="if(event.key==='Enter') sendClassChatMessageAsStudent()">
                <button class="chat-send-btn" onclick="sendClassChatMessageAsStudent()">Send</button>
            </div>
        </div>`;
...
function renderClassChatParticipants(containerId, students, teachers) {
    ...
    students.forEach(s => {
        ...
        html += `<span class="class-chat-chip" title="${s.name} (Student)"><span class="class-chat-chip-avatar student">${initials}</span>${s.name}</span>`;
    });
    teachers.forEach(t => {
        const initials = t.name.split(' ').map(w => w[0]).slice(0,2).join('').toUpperCase();
        html += `<span class="class-chat-chip" title="${t.name} (Teacher)"><span class="class-chat-chip-avatar teacher">${initials}</span>${t.name}</span>`;
    });
...
function renderClassChatThread(boxId, grade, section) {
    ...
    box.innerHTML = '';
    if (!msgs.length) {
        box.innerHTML = '<div class="empty-state" style="margin:auto;padding:16px;">No messages yet — say hello to your class.</div>';
        return;
    }
    msgs.forEach(m => {
        const row = document.createElement('div');
        row.className = `msg-row ${m.senderRole === 'teacher' ? 'recv' : (m.senderId === (currentActiveStudentSession && currentActiveStudentSession.id) ? 'sent' : 'recv')}`;
        row.innerHTML = `<div class="bubble">
            <div style="font-size:11px; font-weight:700; opacity:0.75; margin-bottom:3px; color:${m.senderRole === 'teacher' ? 'var(--gold-light)' : 'inherit'};">${m.senderName}${m.senderRole === 'teacher' ? ' (Teacher)' : ''}</div>
            ${m.content}
...
function renderStudentMessagesThread() {
    ...
    if (!msgs.length) { box.innerHTML = '<div class="empty-state" style="padding-top:30px;">Conversation channel opened safely.</div>'; return; }
...
function toggleStudentVoiceRecordingSim() {
    ...
    if (!isVoiceRecordingActive) {
        ...
        btn.classList.add('voice-active'); btn.textContent = '🛑';
    } else {
        isVoiceRecordingActive = false;
        btn.classList.remove('voice-active'); btn.textContent = '🎤';
        db_addChatMessage(getStudentChatKey(), { type: 'voice', content: 'Voice Note (0:08)', sender: 'student', time: now() });
        renderStudentMessagesThread();
    }
}
...
function buildBubble(msg, myRole) {
    const row = document.createElement('div');
    row.className = `msg-row ${msg.sender === myRole ? 'sent' : 'recv'}`;
    let inner = '';
    if (msg.type === 'text')  inner = `<div>${msg.content}</div>`;
    else if (msg.type === 'image') inner = `<img class="bubble-img" src="${msg.content || ''}" data-id="${msg.id}" data-fallback="${msg.content || ''}">`;
    else if (msg.type === 'voice') inner = `
        <div class="voice-row">
            <span class="voice-icon">🔊</span>
            <div class="voice-bar"><div class="voice-fill"></div></div>
            <span style="font-size:11px;">${msg.content}</span>
        </div>`;
    row.innerHTML = `<div class="bubble">${inner}<span class="bubble-time">${msg.time}</span></div>`;
    return row;
}
...
function renderParentAnnouncementsFeed() {
    const feed = el('parentAnnouncementsFeed'); feed.innerHTML = '';
    if (!stateDatabase.announcements || !stateDatabase.announcements.length) {
        feed.innerHTML = '<div class="placeholder-box">No campus announcements published yet.</div>'; return;
    }
    [...stateDatabase.announcements].reverse().forEach(a => {
        const card = document.createElement('div'); card.className = 'ann-card';
        card.innerHTML = `<div class="ann-meta"><span>🏫 Campus Administration Notice</span><span>${a.date}</span></div>
                          <h3>${a.title}</h3><p>${a.desc}</p>
                          ${a.image || a.hasImage ? `<div class="ann-img-wrap"><img src="${a.image || ''}" data-id="${a.id}-img" data-fallback="${a.image || ''}"></div>` : ''}`;
        feed.appendChild(card);
    });
...
/* ═══════════════════════════════════════════════════════════════════════════
   MODULE 3: EMERGENCY & SAFETY COMMAND CENTER CONTROLLERS
   ═══════════════════════════════════════════════════════════════════════════ */
        ...
        if (!emgs.length) {
            ...
            emgs.slice().reverse().forEach(e => {
                ...
                card.innerHTML = `
                    ...
                    <div style="font-size:11.5px; opacity:0.8; margin-bottom:6px; color:#fecaca;">Audience: ${escapeHTML(e.target)}</div>
                    <div style="font-size:12.5px; color:#fff;">"${escapeHTML(e.message)}"</div>
                `;
                historyBox.appendChild(card);
            });
        }
...
function sendPrincipalEmergencyBroadcast() {
    const level = el('prcEmgLevel') ? el('prcEmgLevel').value : 'Emergency Alert';
    ...
    // Also dispatch to main announcements feed so parents and teachers see it
    ...
    alert(`🚨 Emergency Broadcast dispatched successfully to ${target}!`);
    renderPrincipalEmergency();
}
